...
namespace OCA\OpenIdConnect\Middleware;

use OCA\OpenIdConnect\AuthInfo;
	...
	public function beforeController($controller, $methodName){
        ...
        if ($this->reflector->hasAnnotation('SSOCORS') &&
            ...
            if(!\OC::$server->getSystemConfig()->getValue("sso_one_time_password")) {
                $tokenVaildator = \OCA\OpenIdConnect\RequestManager::send(\OCA\OpenIdConnect\IOpenIdConnectRequest::VALIDTOKEN, $authInfo);
                if (!$tokenVaildator) { 
                    throw new SecurityException('Token expired!', Http::STATUS_UNAUTHORIZED);
                }
            }
            $userInfo = \OCA\OpenIdConnect\RequestManager::getRequest(\OCA\OpenIdConnect\IOpenIdConnectRequest::INFO);

			$this->session->logout();
			if(!\OCA\OpenIdConnect\Util::login($userInfo,$authInfo)) {
